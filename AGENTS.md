...
- Yerel Windows fiyat scheduler: Kripto 5 dk + Portfolio 30 dk + ABD 10 dk için `scripts/windows/register-local-price-sync-tasks.ps1` + `docs/LOCAL-WINDOWS-PRICE-SCHEDULE.md`.
...
- **Crypto sync** (CoinGecko; XAUT/PAXG emtia token; PH yazılmaz): `crypto-sync.yml`; periyot **Supabase** `pg_cron` ~5 dk → Edge `dispatch-crypto-sync`: `docs/SUPABASE-CRYPTO-SYNC.md`.
- **Portfolio sync** (BIST/döviz/emtia/holdings yurtdışı/push; sync sırasında PH yazılmaz; kripto ayrı iş): `portfolio-sync.yml`; periyot **Supabase** `pg_cron` → Edge `dispatch-portfolio-sync`: `docs/SUPABASE-PORTFOLIO-SYNC.md`.
