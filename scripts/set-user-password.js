 ...
 * E-posta doğrulaması / reset link açma olmadan kullanıcı şifresini admin olarak günceller.
 *
 * Kullanım:
 *   node scripts/set-user-password.js <email> <new_password>
 *
 * Örnek:
 *   node scripts/set-user-password.js hasim_o@yahoo.com YeniSifre123!
 */
const fs = require('fs');
...
async function main() {
  ...
  if (!email || !newPassword) {
    console.error('Kullanim: node scripts/set-user-password.js <email> <new_password>');
    process.exit(1);
  }
  if (newPassword.length < 6) {
    console.error('Yeni sifre en az 6 karakter olmali.');
    process.exit(1);
  }
  ...
  console.log('Sifre guncellendi ve email dogrulandi:', email);
}
