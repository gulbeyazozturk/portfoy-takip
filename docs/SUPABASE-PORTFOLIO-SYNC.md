...
- **Kademeli eşikler (varsayılan):** günlük artış için `4, 7, 10, 15` %; düşüş için `-4, -7, -10, -15` %. Aynı kullanıcı + varlık + İstanbul günü için **her eşik değeri** en fazla bir bildirim (`public.daily_gain_push_log` satırındaki `threshold`). Aynı cron turunda birden fazla eşik aşılırsa yalnızca **en güçlü yeni eşik** gönderilir; alt eşikler log’a yazılır. Aynı kullanıcının aynı platformdaki birden fazla Expo token’ı varsa (reinstall / preview+store) yalnızca **en son görülen** token’a gider; böylece birebir aynı mesaj iki kez basılmaz.
