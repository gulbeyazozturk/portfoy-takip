...
function holdingUnitPrice(asset: AssetRow, holding: HoldingRow, usdTry: number): number {
  const rawSpot = Number(asset.current_price ?? holding.avg_price ?? 0);
  if (asset.category_id === 'kripto') {
    return kriptoStoredUnitToUsd(rawSpot, usdTry, asset.currency);
  }
  if (asset.category_id === 'fon') {
    return fonUnitNativeTry(asset.current_price, holding.avg_price);
  }
  return rawSpot;
}
...
function buildHoldingMetrics(
  ...
  for (const h of holdings) {
    ...
    const spot = holdingUnitPrice(asset, h, usdTry);
